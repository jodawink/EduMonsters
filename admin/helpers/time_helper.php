...
class TimeHelper {
    ...
    static function prep_for_db($date,$include_time = false) {
        if($include_time){
    ...
    static function interval_days($days , $include_time = false) {
        
        $date_before = TimeHelper::DateTimeAdjusted();

        $NewDate = new DateTime($date_before);
        if ($days > 0) {
            $NewDate->add(new DateInterval('P' . $days . 'D'));
        } else {
            $NewDate->sub(new DateInterval('P' .abs($days) . 'D'));
        }
