...
class Membership {
    ...
    public function masterLoginCheck($session_duration = 0) {
        ...
        if ($this->user->user_id) {
            ...
            if ($u) {
                ...
                $this->validate_known_session($session_duration, $u);
            } else {
                $this->user = new User();
            }
        } else {

            $cookie_user = $this->get_user_from_cookie();

            $this->user = $this->validate_user($cookie_user);

            if ($this->user->user_id && $this->user->is_logged) {
                $this->validate_known_session($session_duration);
            }
        }
    ...
    public function validate_known_session($session_duration, $u = null) {
     ...
     * 
     * @param User $user 
     * @param type $repeated_password
     * @return type
     */
    public function validate_and_register(&$user, $pass, $repeated_password) {
        ...
        if ($u) {
            $response['error'] = "The username <b>" . $user->username . "</b> is already registered!";
            return $response;
        }
        ...
        if (filter_var($user->email, FILTER_VALIDATE_EMAIL)) {
            ...
            if ($u) {
                $response['error'] = "$user->email is already in use";
                return $response;
            }
        } else {
            $response['error'] = "$user->email is not a valid email address";
            return $response;
        }
